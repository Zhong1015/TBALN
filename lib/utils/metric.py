...
def evaluation(scores_, targets_):
        ...
        for k in range(n_class):
            ...
            Np[k] = np.sum(scores >= 0.715)
            Nc[k] = np.sum((scores >= 0.715) * targets)
        Np[Np == 0] = 1
...
def voc_mAP(imagessetfilelist, num, return_each=False):
    ...
    scores_ = seg[:, :class_num] 
    #print(scores_)
    #print(scores_.shape) 
    # Assuming scores are in the same array
    targets_ = gt_label
    #print(targets_)
    #print(targets_.shape)
    OP, OR, OF1, CP, CR, CF1 = evaluation(scores_, targets_)
    

    if return_each:
        return mAP, aps, OP, OR, OF1, CP, CR, CF1
