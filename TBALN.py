...
def parser_args():
    ...
    parser.add_argument('--dataname', help='dataname', default='coco14', choices=['coco14', 'voc2012', 'nus_wide'])
    parser.add_argument('--dataset_dir', help='dir of dataset', default='/root/nas-private/Dataset')
    ...
    parser.add_argument('--config', type=str, help='config file',default='config_infer.json')

    parser.add_argument('--output', metavar='DIR',default='./output/voc2012_448/bs16record', 
                        help='path to output folder')
    parser.add_argument('--loss', metavar='LOSS', default='asl', 
                        choices=['asl'],
                        help='loss functin')
    parser.add_argument('--num_class', default=81, type=int,
    ...
    parser.add_argument('--resume', type=str, metavar='PATH',default="/root/nas-private/TBALN/command/output/ResNet_576_MSCOCO14/86.63 coco14_resnet101_bs126_e1-d2_asl-0-2-00_lr00001_lrp01_wd001_AdamW_crop_amp_/model_best.pth.tar",
...
def main():
    ...
    logger = setup_logger(output=args.output, distributed_rank=dist.get_rank(), color=False, name="TBALN")
...
def main_worker(args, logger):
    global best_mAP
    pretrained_cfg = timm.models.create_model(args.backbone).default_cfg
    pretrained_cfg['file'] = r"/root/nas-private/convnextv2_base_22k_384_ema.pt"
    # model = timm.create_model(args.backbone,pretrained=True,num_classes=80,pretrained_cfg=pretrained_cfg)
    model = TBALN(args)
    model = model.cuda()
